...
class UnityBuildService:
    ...
    async def _deploy_build(self, job: BuildJob):
        ...
        if build_output.exists() and any(build_output.iterdir()):
            ...
            # Set proper permissions and ownership for web server access
            try:
                # Set directory permissions to 755 (readable/executable by all)
                os.system(f"find {web_dir} -type d -exec chmod 755 {{}} \\;")
                
                # Set file permissions to 644 (readable by all, writable by owner)
                os.system(f"find {web_dir} -type f -exec chmod 644 {{}} \\;")
                
                # Change ownership to www-data for web server access
                os.system(f"chown -R www-data:www-data {web_dir}")
                
                self.logger.info(f"Permissions and ownership set for {web_dir}")
                
            except Exception as e:
                self.logger.warning(f"Permission setting failed (may need sudo): {e}")
